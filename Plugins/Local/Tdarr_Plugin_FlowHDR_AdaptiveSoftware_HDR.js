...
const details = () => ({
  id: 'Tdarr_Plugin_FlowHDR_AdaptiveSoftware_HDR',
  Stage: 'Pre-processing',
  Name: 'FlowHDR Adaptive Software HDR',
  Type: 'Video',
  Operation: 'Transcode',
  Description: 'Adaptive libx265 software encode for HDR with CRF+VBV bitrate constraints, preserving HDR10 metadata.',
  Version: '1.0',
  Tags: 'flowhdr,hdr,libx265,adaptive,software',
  Inputs: [
    ...
    { name: 'crf', type: 'string', defaultValue: '18', inputUI: { type: 'text' },
      tooltip: 'x265 CRF value (0-51, lower = higher quality).' },
    { name: 'preset', type: 'string', defaultValue: 'slow', inputUI: { type: 'text' },
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  r.preset = `-hwaccel cuda ${genpts}<io> -map 0 -c:v libx265 -preset ${preset} -crf ${crf} -g 600 -keyint_min 600 -pix_fmt yuv420p10le -bf 5 -x265-params "${x265Params}" -fps_mode passthrough -c:a copy -c:s copy -max_muxing_queue_size 9999 ${extraMaps}`.trim();
